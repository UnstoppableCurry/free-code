...
export function FileWritePermissionRequest(
  ...
  const titleKey = fileExists
    ? 'ui.permissions.fileWrite.titleOverwrite'
    : 'ui.permissions.fileWrite.titleCreate'
  const questionKey = fileExists
    ? 'ui.permissions.fileWrite.questionOverwrite'
    : 'ui.permissions.fileWrite.questionCreate'
  ...
  return (
    <FilePermissionDialog
      ...
      question={
        <Text>
          {t(questionKey)} <Text bold>{basename(file_path)}</Text>?
        </Text>
      }
      content={
        <FileWriteToolDiff

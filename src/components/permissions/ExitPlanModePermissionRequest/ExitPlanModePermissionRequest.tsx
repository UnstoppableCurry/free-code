...
import type { TranslateFn } from '../../../i18n/translator.js'
...
export function ExitPlanModePermissionRequest({
  ...
  function onImagePaste(
    ...
    const newContent: PastedContent = {
      ...
      filename: filename || t('ui.permissions.exitPlanMode.pastedImage'),
      dimensions,
    }
  ...
  const [currentPlan, setCurrentPlan] = useState(() => {
    ...
    const plan = getPlan()
    return plan ?? t('ui.permissions.exitPlanMode.noPlanFound')
  })
  ...
  async function handleResponse(value: ResponseValue): Promise<void> {
    ...
    if (value === 'ultraplan') {
      ...
      toolUseConfirm.onReject(
        t('ui.permissions.exitPlanMode.ultraplanRefining'),
      )
    ...
    const updatedInput =
      isV2 && !planEditedLocally ? {} : { plan: currentPlan }
    ...
    if (value === 'no') {
      ...
      toolUseConfirm.onReject(
        trimmedFeedback ||
          (hasImages
            ? t('ui.permissions.exitPlanMode.seeAttachedImage')
            : undefined),
  ...
  useLayoutEffect(() => {
    ...
    setStickyFooter(
      ...
      >
        <Text dimColor>
          {t('ui.permissions.exitPlanMode.wouldYouLikeToProceed')}
        </Text>
        <Box marginTop={1}>
          <Select
            options={options}
        ...
        {editorName && (
          <Box flexDirection="row" gap={1} marginTop={1}>
            <Text dimColor>
              {t('ui.permissions.exitPlanMode.ctrlGToEditIn')}
            </Text>
            <Text bold dimColor>
              {editorName}
            </Text>
            ...
            {showSaveMessage && (
              <>
                ...
                <Text color="success">
                  {figures.tick}
                  {t('ui.permissions.exitPlanMode.planSaved')}
                </Text>
              </>
            )}
          </Box>
  ...
  if (isEmpty) {
    ...
    return (
      <PermissionDialog
        color="planMode"
        title={t('ui.permissions.exitPlanMode.titleEmpty')}
        workerBadge={workerBadge}
      >
        <Box flexDirection="column" paddingX={1} marginTop={1}>
          <Text>{t('ui.permissions.exitPlanMode.bodyEmpty')}</Text>
          <Box marginTop={1}>
            <Select
              options={[
                {
                  label: t('ui.permissions.exitPlanMode.optYes'),
                  value: 'yes' as const,
                },
                {
                  label: t('ui.permissions.exitPlanMode.optNo'),
                  value: 'no' as const,
                },
              ]}
  ...
  return (
    <Box flexDirection="column" tabIndex={0} autoFocus onKeyDown={handleKeyDown}>
      <PermissionDialog
        color="planMode"
        title={t('ui.permissions.exitPlanMode.titleReadyToCode')}
        innerPaddingX={0}
        workerBadge={workerBadge}
      >
        <Box flexDirection="column" marginTop={1}>
          <Box paddingX={1} flexDirection="column">
            <Text>{t('ui.permissions.exitPlanMode.heresThePlan')}</Text>
          </Box>
          <Box
            borderColor="subtle"
            borderStyle="dashed"
          ...
          <Box flexDirection="column" paddingX={1}>
            ...
            {isClassifierPermissionsEnabled() &&
              ...
              allowedPrompts.length > 0 && (
                <Box flexDirection="column" marginBottom={1}>
                  <Text bold>
                    {t('ui.permissions.exitPlanMode.requestedPermissions')}
                  </Text>
            ...
            {!useStickyFooter && (
              <>
                <Text dimColor>
                  {t('ui.permissions.exitPlanMode.readyToProceed')}
                </Text>
                <Box marginTop={1}>
                  <Select
                    options={options}
...
export function buildPlanApprovalOptions(
  ...
  const usedLabel =
    usedPercent !== null
      ? t('ui.permissions.exitPlanMode.usedSuffix', { percent: usedPercent })
      : ''
  if (showClearContext) {
    if (feature('TRANSCRIPT_CLASSIFIER') && isAutoModeAvailable) {
      options.push({
        label: t('ui.permissions.exitPlanMode.optYesClearAuto', { usedLabel }),
        value: 'yes-auto-clear-context',
      })
    } else if (isBypassPermissionsModeAvailable) {
      options.push({
        label: t('ui.permissions.exitPlanMode.optYesClearBypass', { usedLabel }),
        value: 'yes-bypass-permissions',
      })
    } else {
      options.push({
        label: t('ui.permissions.exitPlanMode.optYesClearAcceptEdits', {
          usedLabel,
        }),
  ...
  if (feature('TRANSCRIPT_CLASSIFIER') && isAutoModeAvailable) {
    options.push({
      label: t('ui.permissions.exitPlanMode.optYesAuto'),
      value: 'yes-resume-auto-mode',
    })
  } else if (isBypassPermissionsModeAvailable) {
    options.push({
      label: t('ui.permissions.exitPlanMode.optYesBypass'),
      value: 'yes-accept-edits-keep-context',
    })
  } else {
    options.push({
      label: t('ui.permissions.exitPlanMode.optYesAutoAcceptEdits'),
      value: 'yes-accept-edits-keep-context',
    })
  }
  options.push({
    label: t('ui.permissions.exitPlanMode.optYesManualApprove'),
    value: 'yes-default-keep-context',
  })
  if (showUltraplan) {
    options.push({
      label: t('ui.permissions.exitPlanMode.optUltraplan'),
      value: 'ultraplan',
    })
  }
  options.push({
    type: 'input',
    label: t('ui.permissions.exitPlanMode.optNoKeepPlanning'),
    value: 'no',
    placeholder: t('ui.permissions.exitPlanMode.tellClaudeWhatToChange'),
    description: t('ui.permissions.exitPlanMode.shiftTabToApprove'),

...
function SedEditPermissionRequestInner({
  ...
  const noChangesMessage = useMemo(() => {
    if (!fileExists) {
      return t('ui.permissions.sedEdit.fileNotExist')
    }
    return t('ui.permissions.sedEdit.patternNotMatched')
  }, [fileExists, t])

  // Parse input and add _simulatedSedEdit so what user previewed is exactly
  // what gets written (prevents sed/JS regex differences).
  ...
  return (
    <FilePermissionDialog
      ...
      title={t('ui.permissions.sedEdit.title')}
      subtitle={relative(getCwd(), filePath)}
      question={
        <Text>
          {t('ui.permissions.sedEdit.question')}{' '}
